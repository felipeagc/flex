...
static inline std::string log_level_to_string(LogLevel level) {
  switch (level) {
  case L_INFO:
    return "[INFO]";
    break;
...
static inline std::string log_type_to_string(LogType type) {
  switch (type) {
  case L_NONE:
    return "";
    break;
...
static inline void log_print(const std::string &text, LogLevel level = L_INFO,
                             LogType type = L_APP) {
...
static inline void log(LogLevel level, LogType type, const std::string &text) {
  log_print(text, level, type);
}

static inline void log(LogLevel level, const std::string &text) {
  log_print(text, level, L_APP);
}

static inline void log(const std::string &text) {
  log_print(text, L_INFO, L_APP);
}
} // namespace flex
